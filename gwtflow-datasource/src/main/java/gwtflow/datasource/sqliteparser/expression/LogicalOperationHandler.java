...
public class LogicalOperationHandler {
    ...
    private Object getResultImpl(Model tabs,
        ...
        modifiedModel.addTable(left.getTableName());
        modifiedModel.addTable(right.getTableName());
        ...
        for (Model.Row rows1 : tabs.get(left.getTableName()).getRows()) {
            ...
            for (Model.Row rows2 : tabs.get(right.getTableName()).getRows()) {
                Object val2 = rows2.getValue(right.getColumnName());
                if (operation(val1, val2, operType)) {
                    modifiedModel.get(left.getTableName()).addRow(rows1);
                    modifiedModel.get(right.getTableName()).addRow(rows2);
                    modifiedModel.addResultRows(rows1, rows2);
                }
            }
        }
